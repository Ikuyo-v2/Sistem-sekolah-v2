...
<x-alert type="SUCCESS">Eror wak</x-alert>
        ...
        <tbody>
            @foreach ($classes as $schoolClass)
                <tr class="border-b border-[#EFEDE6] hover:bg-[#FAF9F5]">
                    ...
                    <td class="px-5 py-4 font-medium text-[#16213A]">{{ $schoolClass['name'] }}</td>
                    <td class="px-5 py-4">{{ $schoolClass['grade'] }}</td>
                    <td class="px-5 py-4">{{ $schoolClass['major'] }}</td>
                    <td class="px-5 py-4">{{ $schoolClass['homeroom_teacher'] }}</td>
                    <td class="px-5 py-4">
                        <div class="flex justify-end gap-4 text-xs font-medium">
                            <a href="{{ route('classes.show', $schoolClass['id']) }}" class="text-[#16213A] hover:text-[#A16207]">Lihat</a>
                            <a href="{{ route('classes.edit', $schoolClass['id']) }}" class="text-[#16213A] hover:text-[#A16207]">Ubah</a>
                            <form action="{{ route('classes.destroy', $schoolClass['id']) }}" method="POST" onsubmit="return confirm('Hapus data kelas ini dari buku induk?')">
                                @csrf
                                @method('DELETE')
